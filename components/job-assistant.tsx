...
const emptyProfile: Profile = {
  ...
  current_location: '',
  years_experience: '',
  target_roles: '产品经理, AI 产品经理',
  preferred_locations: '上海, 深圳, 杭州',
  expected_salary: '',
  notice_period: '',
};
...
const demoJobs: Job[] = [
  {
    ...
    url: '',
    match_score: 91,
    match_reasons: '["AI 产品经验","地点匹配","行业经历相关"]',
    status: 'new',
...
export function JobAssistant() {
  ...
  const refresh = async (quiet = false) => {
    try {
      ...
      if (!quiet)
        setNotice('当前显示演示数据；保存功能将在数据服务连接后可用。');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredJobs = useMemo(() => {
    ...
    return (data.jobs.length ? data.jobs : demoJobs).filter((job) =>
  ...
  const saveProfile = async (details?: ProfileDetails) => {
    setBusy(true);
    try {
      await callApi({ type: 'save-profile', profile });
      if (details) await callApi({ type: 'save-profile-details', ...details });
      setNotice('全部资料已保存。后续填写申请表将以网页中的记录为准。');
      await refresh(true);
    } catch (error) {
  ...
  const saveAnswer = async () => {
    ...
    try {
      ...
      setNotice('已记住“期望薪资”。下次遇到相同字段会自动填入。');
      setModal(null);
      setAnswer('');
      await refresh(true);
    } catch (error) {
  ...
  const uploadResume = async () => {
    ...
    try {
      ...
      form.append('profile', JSON.stringify(parsed));
      form.append('internships', JSON.stringify(parsed.internships));
      form.append('projects', JSON.stringify(parsed.projects));
      ...
      setNotice(
        `已读取 ${uploadFile.name}，整理出 ${parsed.internships.length} 段实习经历和 ${parsed.projects.length} 个项目。请在“我的资料”中确认并保存。`,
      );
      setModal(null);
      setUploadFile(null);
      await refresh(true);
  ...
  const startSearch = () => {
    ...
    setNotice(
      siteUrl
        ? '已打开招聘官网。找到合适岗位后，把岗位链接加入投递记录即可继续。'
        : '已按当前方向展示匹配岗位；也可以粘贴指定公司的招聘官网。',
    );
    setModal(null);
    setView('jobs');
  };
  ...
  const updateStatus = async (application: Application, status: string) => {
    setBusy(true);
    try {
      await callApi({ type: 'update-status', id: application.id, status });
      setNotice('申请状态和检查时间已更新。');
      await refresh(true);
    } catch (error) {
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto grid min-h-screen max-w-[1580px] grid-cols-1 lg:grid-cols-[224px_minmax(0,1fr)]">
        <aside className="hidden border-r border-border/80 px-5 py-6 lg:flex lg:flex-col">
          ...
          >
            <div className="grid size-10 place-items-center rounded-xl bg-primary text-primary-foreground shadow-[0_6px_18px_rgba(24,72,58,0.18)]">
          ...
          <nav className="mt-10 space-y-1" aria-label="主要导航">
            {navItems.map(({ id, label, icon: Icon }) => (
              <button
                key={id}
                onClick={() => setView(id)}
                className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${view === id ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}
              >
          ...
          <div className="mt-auto rounded-xl border border-border bg-card p-4">
            <div className="mb-3 flex items-center gap-2 text-xs font-medium">
              <Sparkles className="size-4 text-[#b97832]" />
              资料完整度
            ...
            <button
              ...
              className="mt-3 text-left text-[11px] leading-5 text-muted-foreground hover:text-foreground"
            >
              {completeness < 100
                ? '继续补充资料，减少申请时的待确认字段。'
                : '资料已完整，可以开始高效投递。'}
        ...
        <section className="min-w-0 px-4 py-4 sm:px-8 lg:px-10 lg:py-7">
          ...
          <header className="flex flex-wrap items-center justify-between gap-4 border-b border-border/70 pb-5">
            <div>
              ...
              <h1 className="mt-1 font-heading text-2xl font-semibold tracking-[-0.035em] sm:text-[28px]">
                {title}
              </h1>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button size="lg" onClick={() => setModal('search')}>
                <Search /> 开始找岗位
              </Button>
            </div>
          </header>
          ...
          {notice && (
            <div className="mt-4 flex items-start gap-3 rounded-lg border border-[#d7c6a5] bg-[#fff8e9] px-4 py-3 text-xs text-[#72562e]">
              <Sparkles className="mt-0.5 size-4 shrink-0" />
              <p className="flex-1 leading-5">{notice}</p>
          ...
          {view === 'dashboard' && (
            <DashboardView
              ...
              onView={setView}
              onAnswer={() => setModal('answer')}
              onAddJob={addJobToApplications}
              onImport={() => setModal('resume')}
            />
          )}
          {view === 'jobs' && (
            <JobsView
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>导入 PDF 简历</DialogTitle>
            <DialogDescription>
              简历会被读取并整理为可复用字段，实习与项目经历可在“我的资料”中修改；保存后的网页资料优先于
              PDF。建议上传文本型 PDF。
            </DialogDescription>
          </DialogHeader>
          <label className="grid min-h-40 cursor-pointer place-items-center rounded-xl border border-dashed border-primary/35 bg-secondary/50 p-6 text-center hover:bg-secondary">
            <input
              className="sr-only"
              type="file"
            ...
            />
            <span>
              <UploadCloud className="mx-auto mb-3 size-7 text-primary" />
              <strong className="block text-sm">
                {uploadFile ? uploadFile.name : '选择或拖入 PDF 简历'}
              </strong>
              <span className="mt-1 block text-xs text-muted-foreground">
                {uploadFile
                  ? `${(uploadFile.size / 1024 / 1024).toFixed(2)} MB`
                  : '提取联系方式、实习经历与项目经历，单个文件不超过 10MB'}
              </span>
            </span>
          </label>
          <DialogFooter>
            ...
            <Button disabled={!uploadFile || busy} onClick={uploadResume}>
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              可粘贴你指定的网站或某家公司的官方招聘主页。对于需要登录的网站，将在浏览器中打开并由你确认关键操作。
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className={fieldClass}>
              ...
              <Input
                ...
                onChange={(event) =>
                  setProfile({ ...profile, target_roles: event.target.value })
                }
              />
            </label>
            <label className={fieldClass}>
          ...
          <div className="rounded-lg bg-secondary/70 p-3 text-xs leading-5 text-muted-foreground">
            助手只会整理与你偏好匹配的岗位；提交申请前仍会让你确认岗位与资料。
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setModal(null)}>
              取消
            ...
            <Button onClick={startSearch}>
              {siteUrl ? <ExternalLink /> : <Search />}
              {siteUrl ? '打开并查找' : '查看匹配岗位'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>遇到一个新字段</DialogTitle>
            <DialogDescription>
              “期望薪资”尚未保存。回答一次后，以后遇到相同字段会自动填入，你仍可在提交前修改。
            </DialogDescription>
          </DialogHeader>
          <label className={fieldClass}>
            期望薪资
            <Input
              ...
              placeholder="例如：税前 30-35K × 15 薪，可面议"
            />
          </label>
          <DialogFooter>
            <Button variant="outline" onClick={() => setModal(null)}>
            ...
            <Button disabled={!answer.trim() || busy} onClick={saveAnswer}>
...
function MetricCard({
  ...
  return (
    <Card className="gap-2 border-0 bg-card shadow-none ring-border/80">
...
function JobRow({ job, onAdd }: { job: Job; onAdd: (job: Job) => void }) {
  ...
  return (
    <article className="flex flex-col gap-3 border-b border-border/65 px-4 py-4 last:border-0 sm:flex-row sm:items-center">
      ...
      {reasons.length > 0 && (
        <span className="sr-only">匹配原因：{reasons.join('、')}</span>
      )}
    </article>
  );
}
      ...
      <div className="min-w-0 space-y-6">
        <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <MetricCard
            ...
            note="来自目标公司"
          />
          <MetricCard label="进行中" value={active} note="持续跟踪" />
          <MetricCard
            label="面试"
            value={interviews}
            note={interviews ? '记得提前准备' : '等待新机会'}
          />
          <MetricCard
            label="已记住字段"
          ...
          <CardHeader className="flex-row items-center justify-between border-b border-border/70 py-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                演示岗位将在接入目标官网后替换为实时结果
              </p>
            </div>
            <Button variant="ghost" size="sm" onClick={() => onView('jobs')}>
              查看全部 <ChevronRight />
            </Button>
          </CardHeader>
          <CardContent className="px-0">
            {jobs.map((job) => (
              <JobRow key={job.id} job={job} onAdd={onAddJob} />
            ))}
          </CardContent>
        </Card>
      </div>
      <aside className="space-y-4">
        <Card className="gap-0 border-0 bg-primary text-primary-foreground shadow-[0_18px_46px_rgba(22,66,54,0.18)] ring-0">
          <CardHeader className="border-b border-white/10 py-4">
            <div className="flex items-center gap-2 text-xs text-white/70">
              <Sparkles className="size-4 text-[#f2c989]" /> 投递助理
            </div>
          ...
          <CardContent className="space-y-3 py-4">
            <button
              onClick={onAnswer}
              className="w-full rounded-lg bg-white/10 p-3 text-left transition hover:bg-white/15"
            >
              <div className="flex items-start gap-3">
                <span className="mt-0.5 grid size-6 shrink-0 place-items-center rounded-full bg-[#f2c989] text-[11px] font-bold text-[#40321f]">
                  1
                </span>
                <div>
                  <p className="text-sm font-medium">确认期望薪资</p>
            ...
            >
              <div className="flex items-start gap-3">
                <span className="mt-0.5 grid size-6 shrink-0 place-items-center rounded-full bg-white/15 text-[11px] font-bold">
                  2
                </span>
                <div>
                  <p className="text-sm font-medium">处理进行中的投递</p>
                  <p className="mt-1 text-xs leading-5 text-white/60">
                    当前有 {active} 个申请需要持续跟进。
                  </p>
                </div>
              </div>
            </button>
            <Button
              onClick={onAnswer}
              className="mt-1 w-full bg-[#f3d39f] text-[#30281d] hover:bg-[#f0c783]"
              size="lg"
            >
              继续处理 <ChevronRight />
            </Button>
          ...
          <CardContent>
            {resume ? (
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    当前简历 · {formatDate(resume.created_at)}
                  </p>
                </div>
              ...
              >
                ...
                <div>
                  <p className="text-xs font-medium">还没有导入简历</p>
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    上传 PDF 开始整理资料
                  </p>
                </div>
  ...
  return (
    <div className="mt-6 space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button variant="outline" size="lg" onClick={onSearch}>
          <Link2 />
          指定招聘网站
        </Button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
        {jobs.map((job) => (
          <Card
            key={job.id}
            className="gap-4 border-0 shadow-none ring-border/80"
          >
            <CardHeader className="pb-0">
            ...
            <CardContent className="mt-auto">
              <div className="mb-4 flex flex-wrap gap-2">
                {parseReasons(job.match_reasons).map((reason) => (
                  <span
                    key={reason}
    ...
    <div className="mt-6 space-y-6">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {columns.map((status) => (
          <Card
            key={status}
            className="gap-2 border-0 shadow-none ring-border/80"
          >
            <CardHeader className="pb-0">
              <p className="text-xs text-muted-foreground">
            ...
            <CardContent>
              <div className="h-1 rounded-full bg-secondary">
                <div
                  className={`h-full rounded-full ${status === 'draft' ? 'w-1/4' : status === 'submitted' ? 'w-2/4' : status === 'assessment' ? 'w-3/4' : 'w-full'} bg-primary/70`}
                />
              </div>
            </CardContent>
          </Card>
        ...
        <CardContent className="px-0">
          {applications.length ? (
            applications.map((application) => {
              const meta = statusMap[application.status] ?? statusMap.draft;
              return (
                <article
                  key={application.id}
            ...
            <div className="grid min-h-52 place-items-center p-8 text-center">
              <div>
                ...
                <p className="mt-3 text-sm font-medium">还没有投递记录</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  添加第一个岗位，开始跟踪求职进度。
                </p>
              </div>
...
function ProfileView({
  ...
  const inputs: Array<[keyof Profile, string, string]> = [
    ['name', '姓名', '你的姓名'],
        ...
        <CardContent className="grid gap-4 py-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2 flex justify-end">
            <Button disabled={busy} onClick={onSave}>
              {busy && <LoaderCircle className="animate-spin" />}保存个人资料
            </Button>
          </div>
        </CardContent>
      </Card>
